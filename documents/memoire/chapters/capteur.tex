...
La cadence du sportif est déterminée grâce à ce module. En se basant sur les données de l'accéléromètre, qui est capable de déterminer les accélérations sur les 3 axes x,y et z, il est possible de déterminer le moment où un pas est effectué et donc de les compter. 

Afin de pouvoir facilement compter le nombre de pas, l'accéléromètre LSM303AGR est configuré en utilisant le système de seuil qu'il propose. Cela permet de déclencher des interruptions lorsqu'une accélération d'une certaine force est détectée sur les axes.
...
Un exemple d'utilisation du driver UBloxEVA8M est proposé ci-dessous. \todo{Le principe d'utilisation et la configuration d'une fonction qui est appelée par le driver lorsqu'un nouveau message est reçu, une fonction callback.} Dans cette fonction, l'utilisateur peut vérifier de quel type de message il s'agit. Dans le cas où le message l'intéresse, il peut ensuite utiliser les structures permettant le décodage des différents messages.
