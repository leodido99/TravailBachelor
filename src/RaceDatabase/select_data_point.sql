...
SELECT data_point_id, competitor_id, competition_id, sequence, time_stamp, ST_X(position) as lat, ST_Y(position) as lon, heart_rate, cadence, nb_satellites, position_dop, status FROM race_tracker.data_point WHERE data_point.competition_id = 1 ORDER BY sequence;
