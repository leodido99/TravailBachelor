-- Course Test Planeyse
INSERT INTO race_tracker.competitor_registration (competitor_id, competition_id, sensor_id, bib_number) VALUES (1, 1, x'BEEF'::int, 5);
INSERT INTO race_tracker.competitor_registration (competitor_id, competition_id, sensor_id, bib_number) VALUES (2, 1, x'BEEF'::int, 13);
-- Fyne Tera 2018
INSERT INTO race_tracker.competitor_registration (competitor_id, competition_id, sensor_id, bib_number) VALUES (1, 2, x'BEEF'::int, 102);
INSERT INTO race_tracker.competitor_registration (competitor_id, competition_id, sensor_id, bib_number) VALUES (2, 2, x'BEEF'::int, 87);
-- Course Test Valangines
INSERT INTO race_tracker.competitor_registration (competitor_id, competition_id, sensor_id, bib_number) VALUES (1, 3, x'BEEF'::int, 44);
INSERT INTO race_tracker.competitor_registration (competitor_id, competition_id, sensor_id, bib_number) VALUES (2, 3, x'BEEF'::int, 28);

-- Test race #1
-- Inactive
--INSERT INTO race_tracker.competitor_registration (competitor_id, competition_id, sensor_id, bib_number) VALUES (1, 1, x'BEEF'::int, 65);
-- Active
--INSERT INTO race_tracker.competitor_registration (competitor_id, competition_id, sensor_id, bib_number) VALUES (2, 4, x'BEEF'::int, 99);
--INSERT INTO race_tracker.competitor_registration (competitor_id, competition_id, sensor_id, bib_number) VALUES (1, 4, x'DEAD'::int, 77);
