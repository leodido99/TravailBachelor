...
void lora_pkt_fwd_parser::parse(uint8_t* data, int size) {
	if (size >= LORA_PKT_MIN_SIZE) {
		...
		std::memcpy(&this->protocol_version, &data[0], sizeof(this->protocol_version));
 ...
 * @return int
 */
int lora_pkt_fwd_parser::get_protocol_version() {
 ...
 * @return uint16_t
 */
uint16_t lora_pkt_fwd_parser::get_random_token() {
 ...
 */
lora_pkt_fwd_types lora_pkt_fwd_parser::get_pkt_type() {
 ...
 * @return uint8_t*
 */
uint8_t* lora_pkt_fwd_parser::get_pkt_data() {
 ...
 * @return int
 */
int lora_pkt_fwd_parser::get_pkt_data_size() {
